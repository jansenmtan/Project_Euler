...
#qpy:console
'''
A* to the rescue! (Thank God I watched that one Computerphile video!)

A* needs a destination node :(
'''
...
look_ahead = int(len(tri_map) // len(tri_map) ** 0.5)
print(look_ahead)

'''
REMINDER TO INSERT LOOKAHEAD ROUTINE
'''

#using look ahead and brute force
route = [[0,0]]
while route[-1][1] != len(tri_map) - 1:
    if route[-1][1] + look_ahead < len(tri_map):
