...
#moving left required for builders
def move_l(matrix,current_coord):
    next_coord = [current_coord[0] - 1,current_coord[1]]
    if can_move(matrix,next_coord):
        return next_coord
    return current_coord
...
def path_count(length,width):
    ...
    while target_column > 1:        
        for y_pos in range(width - 1):
            """
            I need to include this case:
            x x x w w = 2
            o o x x w = 1
            o o o x w = 1
            o o o x w = 1
            o o o x x
            USE BASE X TO BUILD WALLS
            until all in column are equal to first row, add
            """
            x_pos_list = []
...
for m in range(20):
    n = m + 1
    print("{}: {}".format(n,int(path_count(n,n))))